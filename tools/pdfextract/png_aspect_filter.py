...
(height > 3x width OR width > 3x height) to a target directory.
Landscape images (width > height) are rotated 90° clockwise before moving.

Usage:
    python png_aspect_filter.py <source_dir> <target_dir> [--preserve-structure] [--dry-run]
...
def is_extreme_aspect_ratio(width, height, threshold=3.0):
    """
...
def filter_pngs_by_aspect_ratio(source_dir, target_dir, threshold=3.0, preserve_structure=False, dry_run=False):
    """
    ...
    Args:
        ...
        threshold: Aspect ratio threshold (default: 3.0)
    ...
    for png_file in sorted(png_files):
        ...
        if is_extreme_aspect_ratio(width, height, threshold):
            ...
            rotate_note = " (will rotate 90° clockwise)" if width > height and not dry_run else ""
            
            print(f"  → Extreme aspect ratio detected: {width}x{height} ({aspect_ratio:.2f}:1, {orientation}){rotate_note}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Scan PNG files and move images with extreme aspect ratios (height > 3x width OR width > 3x height) to target directory. Landscape images (width > height) are rotated 90° clockwise.',
...
Examples:
  ...
  %(prog)s ./images ./filtered_images --threshold 4.0
        """
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help='Aspect ratio threshold (default: 3.0, meaning height > 3x width OR width > 3x height)'
    )
