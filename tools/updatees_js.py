...
if '<i class="bi bi-arrows-collapse"></i> Merge Selected ROMs' not in text:
    text = text.replace('                <a class="dropdown-item" href="#" onclick="gameManager.fillSortnameForSelected()">\n                    <i class="bi bi-sort-alpha-down"></i> Fill SortName\n                </a>\n                <div class="dropdown-divider"></div>', merge_link)
    
merge_func = """    async deleteSelectedGames() {
...
if 'showMergeRomsModal()' not in text:
    # We replace deleteSelectedGames to inject just above it. But watch out since there's async deleteSelectedGames(games). 
    # Let's substitute the whole deleteSelectedGames method
    import re
    text = re.sub(
        r'    async deleteSelectedGames\(\) \{\n.*?this\.showDeleteConfirmation\(selectedGames\);\n    \}', 
        merge_func, 
        text, 
        flags=re.DOTALL
    )
