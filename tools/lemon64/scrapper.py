...
class Lemon64Scraper:
    ...
    def parse_rating_from_images(self, soup: BeautifulSoup) -> Optional[float]:
        ...
        for img in images:
            ...
            # Extract filename from src (e.g., "/assets/images/score/7.png" -> "7.png")
    ...
    def extract_game_info(self, soup: BeautifulSoup, game_url: str, list_data: Optional[Dict] = None) -> Optional[Dict]:
        ...
        if not game_info.get('name'):
            ...
            if title_tag:
                ...
                game_info['name'] = re.sub(r'\s*-\s*Lemon 64.*$', '', title_text, flags=re.I).strip()
    ...
    def scrape_offset_pages(self):
        """Scrape all pages from offset 0 to 9240 (increment 40)"""
        games_scraped = 0
    ...
    def run_scraper(self, resume: bool = True):
        """Run the scraper"""
        print("🚀 Starting Lemon64 scraper...")
